...
async function loadCachedManifest() {
    let entry;
    try {
        const stored = await browser.storage.local.get(MANIFEST_CACHE_KEY);
    ...
    if (!result.ok) {
        ...
        try {
            await browser.storage.local.remove(MANIFEST_CACHE_KEY);
        } catch {
...
async function verifyIntegrity() {
    ...
    if (signedData) {
        ...
        if (result.sequence > result.lastKnownSequence) {
            await browser.storage.local.set({ lastKnownSequence: result.sequence });
...
// Run verification when the background context starts. Gated: a Firefox
// event page is torn down when idle, and each restart used to mean another
