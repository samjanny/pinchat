...
function validateResourceAgainstManifest(element) {
    ...
    if (isInternalResource(path)) {
        ...
        if (!expectedSRI) {
            ...
            // (chrome.runtime.sendMessage callback). Until the manifest lands,
...
function showWarningOverlay(mismatches = [], isUnauthorized = false) {
    ...
    // Create host element - NO inline styles, everything via Shadow DOM
    ...
    // Load external CSS from extension (bypasses page CSP)
    const cssUrl = chrome.runtime.getURL('warning.css');

    // All content inside Shadow DOM, CSS loaded externally. The skeleton is
    // a string literal with no interpolation; every dynamic value below goes
    // in through DOM properties, never through markup.
    shadow.innerHTML = `
        <link rel="stylesheet">
        <div class="overlay">
